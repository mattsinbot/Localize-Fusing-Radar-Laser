...
class FusionEKF(object):
	...
	def process_est(self, meas):
		if not self._is_initialized:
			...
			print("process_est is initialized in fusion_ekf class")
			return

		# While already initialized
		if meas[0] == "R":
			dt = (meas[4] - self._prev_timestmp) / 1000000.0  # convert time in seconds
			self._prev_timestmp = meas[4]
		...
		# set the process covariance matrix self._ekf._Q (this is a function of dt)
